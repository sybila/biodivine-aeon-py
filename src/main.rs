...
fn main() {
    ...
    for (k, v) in classes {
        println!(
            "{:?}: {:?}",
            k,
            v.approx_cardinality() / stg.unit_colors().approx_cardinality()
        )
    }
}
...
pub fn get_mapk_phenotypes(graph: &SymbolicAsyncGraph) -> Vec<(String, GraphVertices)> {
    ...
    let other_space = graph
        .mk_unit_vertices()
        .minus(&apoptosis_space)
        .minus(&proliferation_space)
        .minus(&growth_arrest_space)
        .minus(&no_decision_space);
